...
_UPSERT_KEEP_EXISTING_SQL = ",\n".join(
    f"                       {col} = COALESCE(sessions.{col}, excluded.{col})" for col in (
        ...
        "owner_principal", "git_repo_root", "origin_json", "display_name",
    )
)
...
class SessionSessionsMixin:
    ...
    def _insert_session_row(
        ...
        owner_principal: Optional[str] = None, git_repo_root: str = None,
        origin_json: str = None, display_name: str = None,
    ) -> None:
...
""" + _UPSERT_KEEP_EXISTING_SQL,
                (
                    ...
                    parent_session_id, cwd, profile_name, owner_principal, git_repo_root,
                    origin_json, display_name, time.time(),
                ),
            )
